...
$sql = "SELECT * FROM aluno WHERE estagiando = 1 ORDER BY nome";
...
<head>
    ...
    <title>listar estagiários</title>
    <?php
...
<body>
    ...
    <ul class="sidenav">
        ...
        <li><a class="active" href="listarEstagiario.php?id=<?php echo $id; ?>"><i class="fa-solid fa-user"></i> Estagiários</a></li>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="">
                        <p><input type="text" name="pesquisa" placeholder="Digite o nome do aluno em estágio"></p>
                        <button type="submit"><i class="fa-solid fa-magnifying-glass"></i> Pesquisar</button>
                        <?php
                        if ($_POST) {
                            if (isset($_POST['pesquisa'])) {
                                $pesquisa = $_POST['pesquisa'];
                                $sql = "SELECT * FROM aluno WHERE nome LIKE '%$pesquisa%' AND estagiando = 1 ORDER BY nome";
                            } else {
                                $sql = "SELECT * FROM aluno WHERE estagiando = 1 ORDER BY nome";
                                $pesquisa = "";
                            }
                        ...
                        ?>
                                        <tr>
                                <td>
                                    <?php $msgNum = ($resPesquisa['nome']) ? $resPesquisa['nome'] : '<span style="color:red;">Não informado</span>'; echo $msgNum; ?>
                                </td>
                                <td>
                                    <?php $msgNum = ($resPesquisa['nome_guerra']) ? $resPesquisa['nome_guerra'] : '<span style="color:red;">Não informado</span>'; echo $msgNum; ?>
                                </td>
                                <td>
                                    <?php $msgNum = ($resPesquisa['numero']) ? $resPesquisa['numero'] : '<span style="color:red;">Não informado</span>'; echo $msgNum; ?>
                                </td>
                                <td>
                                    <?php
                                    if ($resPesquisa['estagiando'] == 1) {
                                        echo '<span style="color:green; text-decoration: bold;">Sim</span>';
                                    } else {
                                ...
                                <td>
                                    <?php $msgNum = ($resPesquisa['graduacao']) ? $resPesquisa['graduacao'] : '<span style="color:red;">Não informado</span>'; echo $msgNum; ?>
                                </td>
                                <td>
                                    <?php
                                    if ($resPesquisa['id_pelotao'] == 1) {
                                        echo 'Águia';
                                    } else if ($resPesquisa['id_pelotao'] == 2) {
                                        echo 'Falcão';
                                    } else if ($resPesquisa['id_pelotao'] == 3) {
                                        echo 'Tigre';
                                    } else if ($resPesquisa['id_pelotao'] == 4) {
                                        echo 'Leão';
                                    } else if ($resPesquisa['id_pelotao'] == 5) {
                                        echo 'Cobra';
                                    } else {
